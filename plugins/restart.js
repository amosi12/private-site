...
malvin({
    ...
    desc: 'Restart the NOVA-XMD bot system',
    category: 'system',
    react: '♻️',
    filename: __filename,
    ownerOnly: true
...
`.trim(), {
            contextInfo: {
                ...
                forwardedNewsletterMessageInfo: {
                    ...
                    newsletterName: config.OWNER_NAME || 'nova',
                    serverMessageId: 143
                }
            }
        });
...
2. If not using PM2, ensure Docker restart policy is set:
   \`\`\`bash
   docker run -d --restart=always --name nova xmd 
   \`\`\`
3. Contact @${config.OWNER_NAME || 'NOVA-XMD'} for support.
`.trim(), {
            contextInfo: {
                mentionedJid: [sender],
                forwardingScore: 999,

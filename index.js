...
async function loadSession() {
  try {
    ...
    if (config.SESSION_ID.startsWith("NOVA~")) {
      ...
      const base64Data = config.SESSION_ID.replace("NOVA~", "");
      ...
      const megaFileId = config.SESSION_ID.replace("NOVA~", "");
      ...
      throw new Error("Invalid SESSION_ID format. Use 'NOVA~' for base64 or 'NOVA~' for MEGA.nz");
    }
  } catch (error) {
...
try {
  ...
  const repoUrl = "https://github.com/novaxmd/NOVA-XMD";
...
// Follow newsletters
      const newsletterChannels = [                      "120363382023564830@newsletter",
        "120363382023564830@newsletter",
        "120363382023564830@newsletter",
        ];
      let followed = [];
...
BotActivityFilter(malvin);	
 ...
 /// READ STATUS       
  ...
  const ownerNumbers = ["255767862457", "255741752020", "255772341432"];
